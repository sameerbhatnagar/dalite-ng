...
function _getReputation() {
  _getReputation = _asyncToGenerator(
  /*#__PURE__*/
  regenerator.mark(function _callee2() {
    var postData, req, resp, data;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            req = buildReq(postData, "post");
            _context2.next = 4;
            return fetch(model.reputationUrl, req);
...
function reputationView(container, reputation) {
  ...
  name.textContent = "".concat(reputation.full_name);
...
function _init() {
  ...
  /*#__PURE__*/
  regenerator.mark(function _callee3(url, reputationType, id) {
...
function initModel$1(reputationUrl, reputationType, id) {
  model$1 = {
...
function getReputation$1() {
  return _getReputation$1.apply(this, arguments);
}

function _getReputation$1() {
  _getReputation$1 = _asyncToGenerator(
  /*#__PURE__*/
  regenerator.mark(function _callee2() {
    var postData, req, resp, data;
    return regenerator.wrap(function _callee2$(_context2) {
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            postData = {
              reputation_type: model$1.reputationType,
              id: model$1.id
            };
            req = buildReq(postData, "post");
            _context2.next = 4;
            return fetch(model$1.reputationUrl, req);
          ...
          case 7:
            data = _context2.sent;
            model$1.reputations = data.reputations.map(function (r) {
              return {
                name: r.name,
...
function toggleReputationList$1() {
  model$1.reputationListOpen = !model$1.reputationListOpen; // reputationListView();
...
function view$1() {
  iconView$1();
  reputationListView$1();
}

function iconView$1() {
  var _document$querySelect;

  (_document$querySelect = document.querySelector(".reputation-icon")) === null || _document$querySelect === void 0 ? void 0 : _document$querySelect.classList.remove("reputation-icon--hidden");
}

function reputationListView$1() {
  var container = document.querySelector(".reputation-icon__list");

  if (!container) {
    throw new Error("The element with class .reputation-icon__list is missing");
  }

  clear(container);
  model$1.reputations.forEach(function (reputation) {
    reputationView$1(container, reputation);
  });
  toggleReputationView();
}

function reputationView$1(container, reputation) {
  var name = document.createElement("div");
  name.classList.add("reputation-icon__list__name");
  name.textContent = "".concat(reputation.full_name);
  name.title = reputation.description;
  container.appendChild(name);
  container.appendChild(progressView(reputation.name, reputation.reputation, reputation.badgeThresholds, reputation.badgeColour));
}

function progressView(name, reputation, badgeThresholds, badgeColour) {
  var container = document.createElement("div");
  container.classList.add("student-badge");
  var scale = 2;
  var height = 8;
  var svg = select(container).append("svg").attr("viewBox", "0 0 ".concat(100 * scale, " ").concat(20 * scale)).append("g");
  svg.append("text").attr("data-val", Math.round(reputation * 100)).attr("dominant-baseline", "central").attr("x", 5 * scale).attr("y", 10 * scale).attr("class", "fill-primary student-badge__reputation").attr("font-size", 8 * scale).text(0);
  svg.append("rect").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", 75 * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", model$1.mdcThemePrimary).attr("stroke-width", 1).attr("fill", "none");
  svg.append("rect").attr("class", "student-badge__bar").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", Math.min(75 * reputation, height / 2) * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", model$1.mdcThemePrimary).attr("stroke-width", 1).attr("fill", model$1.mdcThemePrimary);
  var badgeArc = arc().innerRadius(height * 5 / 32 * scale).outerRadius(height / 2 * scale).padAngle(0.9).padRadius(height / 8 * scale);
  var badgeArcs = pie()([2, 1, 1]);
  badgeThresholds.forEach(function (threshold) {
    var badge = svg.append("g").attr("transform", "translate(" + "".concat((20 + threshold * 75 - height / 2) * scale, ", ") + "".concat(10 * scale) + ")" + " rotate(180)");

    if (reputation < threshold) {
      badge.append("title").text("Next badge at ".concat(threshold * 100));
    } else {
      badge.append("title").text("Obtained at ".concat(threshold * 100));
    }

    badge.append("circle").attr("cx", 0).attr("cy", 0).attr("r", height / 2 * scale).attr("fill", "#ffffff");
    badge.selectAll("arc").data(badgeArcs).enter().append("g").attr("class", "arc").append("path").attr("d", badgeArc).attr("fill", badgeColour).attr("fill-opacity", reputation < threshold ? 0.5 : 1);
  });
  return container;
}
...
function toggleReputationView() {
  ...
  if (model$1.reputationListOpen) {
    var _document$querySelect2;

    (_document$querySelect2 = document.querySelector(".reputation-icon")) === null || _document$querySelect2 === void 0 ? void 0 : _document$querySelect2.classList.add("reputation-icon--open");
  } else {
    var _document$querySelect3;

    (_document$querySelect3 = document.querySelector(".reputation-icon")) === null || _document$querySelect3 === void 0 ? void 0 : _document$querySelect3.classList.remove("reputation-icon--open");
  }

  var counts = document.getElementsByClassName("student-badge__reputation");
  var bars = document.getElementsByClassName("student-badge__bar");

  var _loop = function _loop(i) {
    var reputation = model$1.reputations[i].reputation;
    var count = select(counts[i]);
    var bar = select(bars[i]);

    if (model$1.reputationListOpen) {
      count.transition().duration(duration).ease(cubicInOut).tween("text", function () {
        var interpolate = interpolateValue(this.textContent, // eslint-disable-line
        Math.round(reputation * 100));
        return function (t) {
          this.textContent = Math.round(interpolate(t)); // eslint-disable-line
        };
      });
      bar.transition().duration(duration).ease(cubicInOut).attr("width", 75 * scale * reputation);
    } else {
      count.transition().duration(duration).ease(cubicInOut).tween("text", function () {
        var interpolate = interpolateValue(this.textContent, 0); // eslint-disable-line

        return function (t) {
          this.textContent = Math.round(interpolate(t)); // eslint-disable-line
        };
      });
      bar.transition().duration(duration).ease(cubicInOut).attr("width", Math.min(75 * reputation, height / 2) * scale);
    }
  };

  for (var i = 0; i < model$1.reputations.length; i++) {
    _loop(i);
  }
}
/*************/
...
function addEventListeners$1() {
  addReputationListOpenListener$1();
}

function addReputationListOpenListener$1() {
  var _document$querySelect4, _document$querySelect5, _document$body;

  (_document$querySelect4 = document.querySelector(".reputation-icon")) === null || _document$querySelect4 === void 0 ? void 0 : _document$querySelect4.addEventListener("click", function (event) {
    event.stopPropagation();
  });
  (_document$querySelect5 = document.querySelector(".reputation-icon__icon")) === null || _document$querySelect5 === void 0 ? void 0 : _document$querySelect5.addEventListener("click", function (event) {
    toggleReputationList$1();
  });
  ...
  regenerator.mark(function _callee3(url, reputationType, id) {
    return regenerator.wrap(function _callee3$(_context3) {
      while (1) {
        switch (_context3.prev = _context3.next) {
          case 0:
            initModel$1(url, reputationType, id);
            _context3.next = 3;
            return update$1();
...
exports.teacherInit = init;
exports.studentInit = init$1;
