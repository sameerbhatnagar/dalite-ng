...
function _getReputation() {
  _getReputation = _asyncToGenerator(function* () {
    ...
    model.reputations = data.reputations.map(function (r) {
      return {
        name: r.name,
...
function toggleReputationList() {
  model.reputationListOpen = !model.reputationListOpen; // reputationListView();
...
function progressView(name, reputation, badgeThresholds, badgeColour) {
  ...
  svg.append("text").attr("data-val", reputation).attr("dominant-baseline", "central").attr("x", 5 * scale).attr("y", 10 * scale).attr("class", "fill-primary student-badge__reputation").attr("font-size", 8 * scale).text(0);
  svg.append("rect").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", 75 * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", model.mdcThemePrimary).attr("stroke-width", 1).attr("fill", "none");
  svg.append("rect").attr("class", "student-badge__bar").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", Math.min(75 * reputation / 100, height / 2) * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", model.mdcThemePrimary).attr("stroke-width", 1).attr("fill", model.mdcThemePrimary);
  ...
  badgeThresholds.forEach(function (threshold) {
    var badge = svg.append("g").attr("transform", "translate(" + "".concat((20 + threshold * 75 / 100 - height / 2) * scale, ", ") + "".concat(10 * scale) + ")" + " rotate(180)");
...
function toggleReputationView() {
  ...
  var _loop = function _loop(i) {
    var reputation = model.reputations[i].reputation;
    ...
    if (model.reputationListOpen) {
      ...
      bar.transition().duration(duration).ease(cubicInOut).attr("width", 75 * scale * reputation / 100);
    } else {
      count.transition().duration(duration).ease(cubicInOut).tween("text", function () {
      ...
      bar.transition().duration(duration).ease(cubicInOut).attr("width", Math.min(75 * reputation / 100, height / 2) * scale);
    }
  };
...
var TeacherReputationHeader =
/*#__PURE__*/
function (_HTMLElement) {
  _inherits(TeacherReputationHeader, _HTMLElement);
  ...
  _createClass(TeacherReputationHeader, [{
    ...
    get: function get() {
      ...
      if (!url) {
        throw new Error("The teacher-reputation-header needs a `reputation-url` attribute");
      }
    ...
    key: "reputationType",
    get: function get() {
      var type = this.getAttribute("reputation-type");

      if (!type) {
        throw new Error("The teacher-reputation-header needs a `reputation-type` attribute");
      }
    ...
    get: function get() {
      ...
      if (!id) {
        throw new Error("The teacher-reputation-header needs a `reputation-id` attribute");
      }
  ...
  function TeacherReputationHeader() {
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(TeacherReputationHeader).call(this));
    ...
    value: function () {
      var _init = _asyncToGenerator(function* (shadow) {
        ...
        function styleView() {
          ...
          style.setAttribute("href", window.location.protocol + "//" + window.location.host + "/static/reputation/css/teacher-header.min.css");
