    ...
    value: function () {
      var _init = _asyncToGenerator(function* (shadow) {
        ...
        function reputationView(list, reputation) {
          ...
          list.appendChild(progressView(reputation.name, reputation.reputation, reputation.badgeThresholds, reputation.badgeColour));
        }
        ...
        function progressView(name, reputation, badgeThresholds, badgeColour) {
          ...
          svg.append("text").attr("data-val", reputation).attr("dominant-baseline", "central").attr("x", 5 * scale).attr("y", 10 * scale).attr("class", "fill-primary badge__reputation").attr("font-size", 8 * scale).text(0);
          svg.append("rect").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", 75 * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", "var(--reputation-colour)").attr("stroke-width", 1).attr("fill", "none");
          svg.append("rect").attr("class", "badge__bar").attr("x", 20 * scale).attr("y", (20 - height) / 2 * scale).attr("height", height * scale).attr("width", Math.min(75 * reputation / 100, height / 2) * scale).attr("rx", height / 2 * scale).attr("ry", height / 2 * scale).attr("stroke", "var(--reputation-colour)").attr("stroke-width", 1).attr("fill", "var(--reputation-colour)");
          badgeThresholds.forEach(function (threshold) {
            var badge = svg.append("g").attr("height", height * scale).attr("width", height * scale).attr("fill", badgeColour).attr("fill-opacity", reputation < threshold ? 0.5 : 1);
