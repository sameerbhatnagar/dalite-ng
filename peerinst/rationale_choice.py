...
rationales), where rationales is a list of pairs (rationale_id,
rationale_text).
...
The callable must have the following attributes:
  ...
  description: A long description explaining how the algorithm chooses
  rationales.

To make an algorithm available to users, make sure to add it to the
"algorithms" dictionary at the end of this file.
"""
...
def _base_selection_algorithm(
    ...
    # Rationales are selected based on those who have not refused to include
    # rationales prior to implementation of TOS
    ...
    # Select a second answer to offer at random.  If the user's answer wasn't
    # correct, the second answer choice offered must be correct.
    if answer_choices[first_choice - 1].correct:
        # We must make sure that rationales for the second answer exist.  The
        # choice is weighted by the number of rationales available.
        other_rationales = all_rationales.exclude(
            first_answer_choice=first_choice
        )
        # We don't use rng.choice() to avoid fetching all rationales from the
        # database.
        try:
            ...
            raise RationaleSelectionError(
                ugettext(
                    "Can't proceed since the course staff did not provide "
                    "example answers."
                )
            )
        second_choice = random_rationale.first_answer_choice
    else:
        # Select a random correct answer.  We assume that a correct answer
        # exists.
...
simple.description = _(
    "The two answer choices presented will include the answer the user "
    "chose. If the user's answer wasn't correct, the second choice will be a "
    "correct answer. If the user's answer was correct, the second choice "
    "presented will be weighted by the number of available rationales, i.e. "
    "an answer that has only a few rationales available will have a low "
    "chance of being shown to the user. Up to four rationales are presented "
    "to the user for each choice, if available. In addition, the user can "
    "choose to stick with their own rationale."
)
...
simple_sequential.description = _(
    "The two answer choices presented will include the answer the user "
    "chose. If the user's answer wasn't correct, the second choice will be "
    "a correct answer. If the user's answer was correct, the second choice "
    "presented will be weighted by the number of available rationales, i.e. "
    "an answer that has only a few rationales available will have a low "
    "chance of being shown to the user. Up to four rationales are presented "
    "to the user for each choice, if available. In addition, the user can "
...
prefer_expert_and_highly_voted.description = _(
    ...
    "chance of being shown to the user. Up to four rationales are presented "
    "to the user for each choice, if available. In addition, the user can "
    "choose to stick with their own rationale. The four rationales will "
    'include at least one "expert" rationale, if available, and at least one '
    "rationale with more than half the maximum number of votes, if available."
)
...
def algorithm_choices():
    ...
    The choices are in a format suitable to be used as the "choices" parameter
    of a model field, i.e. pairs of the form (value, human-readable value).
    """
