...
from peerinst.heartbeat_checks import (
    check_db_query,
    check_staticfiles,
    SanityCheckResult,
)
...
class TestHeartbeatQuery(TestCase):
    ...
    def test_db_query_negative(self):
        with mock.patch(
            "django.db.connection.cursor", side_effect=DatabaseError()
        ):
            result = check_db_query()
...
@override_settings(
    DEFAULT_FILE_STORAGE="peerinst.tests.test_heartbeat.MockStorage"
)
class TestHeartbeatStaticfiles(PatcherBaseTest):
    ...
    def test_staticfiles_positive(self):
        ...
        response.read = mock.MagicMock(
            side_effect=lambda: default_storage.last_uploaded_file_contents
        )
        urlopen = self._add_patcher(
            mock.patch("urllib2.urlopen", return_value=response)
        )
        response = check_staticfiles()
        self.assertEqual(len(default_storage.deleted_files), 1)
        self.assertEqual(
            default_storage.deleted_files, default_storage.saved_files
        )
    ...
    def test_staticfiles_raise_urlopen(self):
        urlopen = self._add_patcher(
            mock.patch(
                "urllib2.urlopen",
                side_effect=urllib.error.URLError("Permission Denied"),
            )
        )
        response = check_staticfiles()
        self.assertEqual(len(default_storage.deleted_files), 1)
