...
function editField(event, type, className) {
  var iconContainer = event.currentTarget.parentNode.parentNode;
  var container = event.currentTarget.parentNode.previousElementSibling;
  var field = event.currentTarget.parentNode.previousElementSibling.firstElementChild;
  var newField;
  ...
  if (type == "text") {
    ...
    console.log("The `editField` function isn 't implemented for type " + type + ".");
    return;
  }

  container.replaceChild(newField, field);
  var iconsDiv = event.currentTarget.parentNode;
  var newIconsDiv = iconsDiv.cloneNode(false);
...
function saveField(event, type, save, className) {
  ...
  var container = event.currentTarget.parentNode.previousElementSibling;
  var field = event.currentTarget.parentNode.previousElementSibling.firstElementChild;
  var url = container.parentNode.parentNode.parentNode.getAttribute("data-update-url");
  ...
  if (type == "text") {
    ...
    console.log("The `saveField` function isn't implemented for type " + type + ".");
    return;
  }
...
function editTextListField(field) {
  ...
  var name = field.getAttribute("name");
  newField.setAttribute("name", name);
  var ul = field.querySelector("ul").cloneNode(true);
...
function editDateTimeField(field) {
  ...
  input.setAttribute("data-old-content", field.textContent);
  newField.append(input);
  var datetime = field.textContent.match(/(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})/).slice(1).map(function (x) {
    return parseInt(x);
  });
  datetime[1] -= 1;
  flatpickr(input, {
    enableTime: true,
    defaultDate: _construct(Date, _toConsumableArray(datetime)),
    dateFormat: "Z",
    altInput: true,
    altFormat: "Y-m-d H:i",
    minDate: "today"
...
function toggleIcons(newIconsDiv, type, toEdit, className) {
  if (toEdit) {
    ...
    editIcon.classList.add("material-icons", "md-24", "mdc-ripple-surface", "icon-list");
    ...
    editIcon.setAttribute("onclick", "group.editField(event, '" + type + "', '" + className + "')");
    newIconsDiv.append(editIcon);
  } else {
    var saveIcon = document.createElement("i");
    saveIcon.classList.add("material-icons", "md-24", "mdc-ripple-surface", "icon-list");
    saveIcon.textContent = "check";
    saveIcon.title = "Save";
    saveIcon.setAttribute("onclick", "group.saveField(event, '" + type + "', true, '" + className + "')");
    var cancelIcon = document.createElement("i");
    cancelIcon.classList.add("material-icons", "md-24", "mdc-ripple-surface", "icon-list");
    cancelIcon.textContent = "close";
    cancelIcon.title = "Cancel";
    cancelIcon.setAttribute("onclick", "group.saveField(event, '" + type + "', false, '" + className + "')");
...
function saveTextField(_x, _x2, _x3, _x4) {
  return _saveTextField.apply(this, arguments);
}

function _saveTextField() {
  _saveTextField = _asyncToGenerator(function* (field, save, className, url) {
    ...
    if (save) {
      var newValue = field.firstChild.value;
      var err = yield updateDetails(name, newValue, url);
...
function saveTextListField(_x5, _x6, _x7, _x8) {
  return _saveTextListField.apply(this, arguments);
}

function _saveTextListField() {
  _saveTextListField = _asyncToGenerator(function* (field, save, className, url) {
    var newField = document.createElement("span");
    var name = field.getAttribute("name");
    newField.setAttribute("name", name);
    var ul = field.firstChild.cloneNode(true);
    var li = field.firstChild.lastChild;
    ul.removeChild(ul.lastChild);

    if (save) {
      var newValue = li.firstChild.value;
      var err = yield updateDetails(name, newValue, url);
...
function saveDateTimeField(_x9, _x10, _x11, _x12) {
  return _saveDateTimeField.apply(this, arguments);
}

function _saveDateTimeField() {
  _saveDateTimeField = _asyncToGenerator(function* (field, save, className, url) {
    var newField = document.createElement("span");
    var name = field.getAttribute("name");
    newField.setAttribute("name", name);
    newField.setAttribute("class", className);
    ...
    if (save) {
      var sentValue = field.firstChild.value;
      var newValue = flatpickr.formatDate(new Date(sentValue), "Y-m-d H:i");
      var err = yield updateDetails(name, sentValue, url);
...
function updateDetails(_x13, _x14, _x15) {
  return _updateDetails.apply(this, arguments);
}

function _updateDetails() {
  _updateDetails = _asyncToGenerator(function* (name, value, url) {
    var data = {
      name: name,
      value: value
    };
...
function initCourseLinking(linkUrl, unlinkUrl, linkTrans, unlinkTrans, linkedTo, unlinkedFrom, error) {
  ...
  [].forEach.call(document.querySelectorAll(".course-toggle-link"), function (el) {
...
function initModel(data) {
  ...
  model = {
    language: ((_document$getElementB = document.getElementById("corner")) === null || _document$getElementB === void 0 ? void 0 : _document$getElementB.getElementsByTagName("text")[0].textContent) == "FR" ? "en-ca" : "fr-ca",
...
function toggleStudentIdNeeded(event, url) {
  var idNeeded = event.currentTarget.checked;
  var data = {
...
function createCollection(_x, _x2, _x3) {
  return _createCollection.apply(this, arguments);
}

function _createCollection() {
  _createCollection = _asyncToGenerator(function* (groupPk, addAssignmentUrl, collectionUpdateUrl) {
    var req = buildReq({
      group_pk: groupPk
    }, "post");
    var resp = yield fetch(addAssignmentUrl, req);
    var data = yield resp.json();
  });
  return _createCollection.apply(this, arguments);
}

function getStudentInformation(_x4) {
...
function _getStudentInformation() {
  _getStudentInformation = _asyncToGenerator(function* (student) {
    var req = buildReq({
      id: student.id
    }, "post");
    var resp = yield fetch(model.urls.getStudentInformationUrl, req);
...
function studentListTableView(student) {
  var criteria = _toConsumableArray(document.querySelectorAll("#student-reputation-table th")).slice(2).map(function (header) {
    return header.getAttribute("name");
  }); // $FlowFixMe


  var table = $("#student-reputation-table").DataTable(); // eslint-disable-line

  table.row.add([student.email, student.lastLogin ? student.lastLogin.toLocaleString(model.language, {
    month: "short",
    // eslint-disable-line
    day: "numeric",
    // eslint-disable-line
  ...
  : ""].concat(_toConsumableArray(criteria.map(function (criterion) {
    return student.criteria[criterion];
  })))).draw();
}
/*************/
...
function addLinkListeners() {
  var links = document.getElementsByClassName("assignment-link");
...
function addRemoveAssignmentListeners() {
...
function addEditListeners() {
  _toConsumableArray(document.getElementsByClassName("edit-btn")).forEach(function (btn) {
...
function addToggleIdListener() {
  document.getElementById("toggle-id-btn").addEventListener("click", function (event) {
...
function initModel$2(data) {
  model$2 = {
    dataLoaded: true,
    showing: document.querySelector("#student-progress").parentNode.parentNode.classList.contains("foldable__unfolded"),
    results: data.map(function (question) {
      return {
...
function toggleStudentProgress() {
  model$2.showing = !model$2.showing;

  if (model$2.dataLoaded) {
    toggleStudentProgressView();
  }
}
/********/
...
function view$1() {
  document.querySelector("#student-progress").parentNode.parentNode.classList.remove("hidden");
...
function questionView(question) {
  ...
  var nStudents = document.createElement("span");
  ...
  timeSpent.classList.add("mdc-list-item__secondary-text", "student-progress__time_taken");
...
function completeView(container, data, total, height, width) {
  ...
  var svg = select(container).append("svg").attr("class", "student-progress-complete").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  var arcBackground = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0).endAngle(2 * Math.PI);
  var arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
  svg.append("path").attr("d", arcBackground).attr("class", "fill-primary").style("opacity", "0.10");
  svg.append("path").datum({
    endAngle: 0
  }).attr("d", arcData).attr("class", "fill-primary student-progress__path");
  svg.append("text").attr("data-count", data).attr("data-total", total).text(0).attr("text-anchor", "middle").attr("dy", 8).attr("class", "fill-primary student-progress__count").attr("font-size", "20px");
  return svg;
}

function correctView(container, data, total, height, width) {
  var radius = Math.min(width, height) / 2;
  var svg = select(container).append("svg").attr("class", "student-progress-correct").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  var colourScale = quantile$$1().domain([0, 1]).range(["#b30000", "#f17f4d", "#339966"]);
  var arcBackground = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0).endAngle(2 * Math.PI);
  var arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
  svg.append("path").attr("d", arcBackground).attr("class", "fill-primary").style("opacity", "0.10");
  svg.append("path").datum({
    endAngle: 0
  }).attr("d", arcData).style("fill", colourScale(0)).attr("class", "student-progress__path");
  svg.append("text").attr("data-count", data).attr("data-total", total).text(0).attr("text-anchor", "middle").attr("dy", 8).style("fill", colourScale(0)).attr("font-size", "24px").attr("class", "student-progress__count");
  return svg;
}

function toggleStudentProgressView() {
  var progress = document.querySelector("#student-progress");
  var complete = progress.querySelectorAll(".student-progress-complete");
  var correct = progress.querySelectorAll(".student-progress-correct");
  Array.from(complete).map(function (svg) {
    animateComplete(svg, !model$2.showing);
  });
  Array.from(correct).map(function (svg) {
    animateCorrect(svg, !model$2.showing);
  });
}

function animateComplete(svg) {
  var reverse = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var path_ = svg.querySelector(".student-progress__path");
  var count_ = svg.querySelector(".student-progress__count");
  var data = count_.getAttribute("data-count");
  var total = count_.getAttribute("data-total");
  var start;
  var end;
  var delay;
  var duration;
  ...
  var radius = Math.min(width, height) / 2;
  var arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
  var path = select(path_).attr("d", arcData);
  var count = select(count_);

  function animation(transition, newAngle) {
    transition.attrTween("d", function (d) {
      var interpolate = interpolateValue(d.endAngle, newAngle);
      var interpolateCount = interpolateValue(start, end);
      return function (t) {
  ...
  function animate() {
    path.transition().delay(delay * Math.random()).duration(duration).ease(cubicInOut).call(animation, 2 * Math.PI * end / total);
  }

  setTimeout(animate, 0);
}

function animateCorrect(svg) {
  var reverse = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  var path_ = svg.querySelector(".student-progress__path");
  var count_ = svg.querySelector(".student-progress__count");
  var data = count_.getAttribute("data-count");
  ...
  function animation(transition, newAngle) {
    transition.attrTween("d", function (d) {
      ...
      return function (t) {
        d.endAngle = interpolate(t);
        var newCount = interpolateCount(t);
...
function initListeners$2() {
  addToggleStudentProgressListener();
}

function addToggleStudentProgressListener() {
  document.querySelector("#student-progress").parentNode.parentNode.querySelector(".foldable--title").addEventListener("click", toggleStudentProgress);
}
/********/
...
function initStudentProgress(url) {
  view$1();
  initListeners$2();
...
function initModel$1(data) {
  model$1 = {
    assignment: {
      ...
      distributionDate: data.assignment.distribution_date ? new Date(data.assignment.distribution_date) : null
    },
    urls: {
...
function distributeAssignment() {
  var button = document.querySelector("#assignment-distribution button");
  button.disabled = true;
  var req = buildReq({}, "post");
  fetch(model$1.urls.distributeAssignment, req).then(function (resp) {
    return resp.json();
  }).then(function (assignment) {
    model$1.assignment = {
      hash: assignment.hash,
      distributionDate: assignment.distribution_date ? new Date(assignment.distribution_date) : null
    };
    distributedView();
    initStudentProgress(model$1.urls.getAssignmentStudentProgress);
  }).catch(function (err) {
    console.log(err);
...
function onQuestionListModified() {
  var btn = document.querySelector("#question-list-save");
  btn.disabled = false;
}

function saveQuestionList() {
  var questions = Array.from(document.querySelectorAll("#question-list .draggable")).map(function (x) {
    return x.getAttribute("data-draggable-name");
  });
  var data = {
    name: "question_list",
    value: questions
  ...
  var req = buildReq(data, "post");
  fetch(model$1.urls.groupAssignmentUpdate, req).then(function () {
    var btn = document.querySelector("#question-list-save");
    btn.disabled = true;
  }).catch(function (err) {
    console.log(err);
...
function sendAssignmentEmail(event) {
  ...
  var email = icon.parentNode.parentNode.querySelector(".student-list--email").getAttribute("data-email");
  var data = {
    email: email
  };
  var req = buildReq(data, "post");
  fetch(model$1.urls.sendStudentAssignment, req).then(function (resp) {
    if (resp.ok) {
...
function distributedView() {
  var span = document.querySelector("#assignment-distribution");
  clear(span);

  if (model$1.assignment.distributionDate) {
    span.textContent = model$1.translations.distributed;
    var datetimeSpan = document.createElement("span");
    datetimeSpan.classList.add("mdc-list-item__secondary-text");
    datetimeSpan.textContent = formatDatetime(model$1.assignment.distributionDate);
    span.append(datetimeSpan);
  } else {
    var button = document.createElement("button");
    button.classList.add("mdc-button", "mdc-button--raised");
    button.textContent = model$1.translations.distribute;
    span.append(button);
    var warning = document.createElement("span");
    warning.classList.add("assignment-distribution__warning");
    warning.textContent = model$1.translations.distributionWarning;
    span.append(warning);
  }
}
/*************/
...
function addDistributeListener() {
  if (!model$1.assignment.distributionDate) {
    document.querySelector("#assignment-distribution button").addEventListener("click", distributeAssignment);
  }
}

function addQuestionListDragListeners() {
  Array.from(document.querySelectorAll(".draggable")).map(function (x) {
    return x.addEventListener("dragenter", function () {
      return onQuestionListModified();
    });
  });
  document.querySelector("#question-list-save").addEventListener("click", saveQuestionList);
}

function addSendAssignmentEmailListeners() {
  Array.from(document.querySelectorAll(".email-btn")).map(function (x) {
    return x.addEventListener("click", function (event) {
      return sendAssignmentEmail(event);
    });
  });
}
...
exports.initCourseLinking = initCourseLinking;
exports.initAssignment = initAssignment;
exports.initDetails = init;

}((this.group = this.group || {}),flatpickr));
