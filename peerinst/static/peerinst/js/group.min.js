...
function editField(event, type, className) {
  ...
  const field = event.currentTarget.parentNode.previousElementSibling.firstElementChild;
  let newField;
...
function saveField(event, type, save, className) {
  ...
  const container = event.currentTarget.parentNode.previousElementSibling;
  const field = event.currentTarget.parentNode.previousElementSibling.firstElementChild;
  const url = container.parentNode.parentNode.parentNode.getAttribute("data-update-url");
...
function editDateTimeField(field) {
  ...
  input.setAttribute("data-old-content", field.textContent);
  newField.append(input);
  const datetime = field.textContent.match(/(\d{4})-(\d{2})-(\d{2})\s+(\d{2}):(\d{2})/).slice(1).map(x => parseInt(x));
  datetime[1] -= 1;
  flatpickr(input, {
    enableTime: true,
...
function completeView(container, data, total, height, width) {
  ...
  const svg = select(container).append("svg").attr("class", "student-progress-complete").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  const arcBackground = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0).endAngle(2 * Math.PI);
  const arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
...
function correctView(container, data, total, height, width) {
  const radius = Math.min(width, height) / 2;
  const svg = select(container).append("svg").attr("class", "student-progress-correct").attr("width", width).attr("height", height).append("g").attr("transform", "translate(" + width / 2 + "," + height / 2 + ")");
  const colourScale = quantile$$1().domain([0, 1]).range(["#b30000", "#f17f4d", "#339966"]);
  const arcBackground = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0).endAngle(2 * Math.PI);
  const arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
  svg.append("path").attr("d", arcBackground).attr("class", "fill-primary").style("opacity", "0.10");
  svg.append("path").datum({
...
function animateComplete(svg, reverse = false) {
  ...
  const radius = Math.min(width, height) / 2;
  const arcData = arc().innerRadius(radius - 5).outerRadius(radius).startAngle(0);
  const path = select(path_).attr("d", arcData);
  const count = select(count_);

  function animation(transition, newAngle) {
    transition.attrTween("d", function (d) {
      const interpolate = interpolateValue(d.endAngle, newAngle);
      const interpolateCount = interpolateValue(start, end);
      return function (t) {
...
function initStudentProgress(url) {
  ...
  fetch(url, req).then(resp => resp.json()).then(function (data) {
...
function distributeAssignment() {
  ...
  fetch(model.urls.distributeAssignment, req).then(resp => resp.json()).then(function (assignment) {
    model.assignment = {
...
function saveQuestionList() {
  const questions = Array.from(document.querySelectorAll("#question-list .draggable")).map(x => x.getAttribute("data-draggable-name"));
  const data = {
    name: "question_list",
    value: questions
  };
...
function sendAssignmentEmail(event) {
  ...
  const email = icon.parentNode.parentNode.querySelector(".student-list--email").getAttribute("data-email");
  const data = {
    email: email
  };
  const req = buildReq(data, "post");
...
function addQuestionListDragListeners() {
  Array.from(document.querySelectorAll(".draggable")).map(x => x.addEventListener("dragenter", () => onQuestionListModified()));
...
function addSendAssignmentEmailListeners() {
  Array.from(document.querySelectorAll(".email-btn")).map(x => x.addEventListener("click", event => sendAssignmentEmail(event)));
}
/********/
