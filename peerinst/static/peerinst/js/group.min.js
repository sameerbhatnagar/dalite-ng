...
function toggleStudentProgress() {
  model$1.showing = !model$1.showing;

  if (model$1.dataLoaded) {
    toggleStudentProgressView();
  }
}
/********/
...
function progressView() {
  ...
  model$1.results.map(function (question) {
...
function toggleStudentProgressView() {
  ...
  Array.from(complete).map(function (svg) {
    animateComplete(svg, !model$1.showing);
  });
  Array.from(correct).map(function (svg) {
    animateCorrect(svg, !model$1.showing);
  });
}
...
function distributeAssignment() {
  ...
  fetch(model.urls.distributeAssignment, req).then(function (resp) {
    return resp.json();
...
function saveQuestionList() {
  ...
  fetch(model.urls.groupAssignmentUpdate, req).then(function () {
    var btn = document.querySelector("#question-list-save");
    btn.disabled = true;
  }).catch(function (err) {
    console.log(err);
...
function sendAssignmentEmail(event) {
  ...
  var req = buildReq(data, "post");
  fetch(model.urls.sendStudentAssignment, req).then(function (resp) {
    if (resp.ok) {
...
function distributedView() {
  ...
  if (model.assignment.distributionDate) {
    ...
    datetimeSpan.textContent = formatDatetime(model.assignment.distributionDate);
...
function _update() {
  _update = _asyncToGenerator(function* () {
    model$2.students.forEach(function (student) {
...
function getStudentInformation(_x) {
...
function _getStudentInformation() {
  _getStudentInformation = _asyncToGenerator(function* (student) {
    ...
    var resp = yield fetch(model$2.urls.getStudentInformationUrl, req);
...
function studentListTableView(student) {
  ...
  table.row.add([student.email, student.lastLogin ? student.lastLogin.toLocaleString(model$2.language, {
    month: "short",
    // eslint-disable-line
    day: "numeric",
    // eslint-disable-line
...
function addLinkListeners() {
  ...
  var _loop = function _loop(i) {
    links[i].addEventListener("click", function (event) {
      window.location.assign(model$2.assignments[i].url);
    });
  };
...
function addRemoveAssignmentListeners() {
  ...
  var _loop2 = function _loop2(i) {
    btns[i].addEventListener("click", function (event) {
      removeAssignment(event, model$2.assignments[i].url);
    });
  };
...
function addToggleIdListener() {
  document.getElementById("toggle-id-btn").addEventListener("click", function (event) {
    toggleStudentIdNeeded(event, model$2.urls.updateUrl);
  });
}
/********/
