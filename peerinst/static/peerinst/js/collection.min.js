...
function init(assignUrl, unassignUrl, assignTrans, unassignTrans, assignedTo, unassignedFrom, error, groupUrl) {
  ...
  function clickAssign(el) {
    ...
    posting.done(function (data) {
      console.info(data);
      document.getElementById(el.getAttribute("id_pk")).innerHTML = unassignTrans;
      document.getElementById(el.getAttribute("id_pk")).classList.add("added");
      document.getElementById(el.getAttribute("id_pk")).classList.remove("removed");
      var dataObjAssigned = {
  ...
  function clickUnassign(el) {
    ...
    posting.done(function (data) {
      ...
      document.getElementById(el.getAttribute("id_pk")).classList.remove("added");
  ...
  [].forEach.call(document.querySelectorAll(".collection-toggle-assign"), function (el) {
  ...
  [].forEach.call(document.querySelectorAll(".md-48"), function (el) {
    el.addEventListener("click", function () {
...
function toggleFollower(pk, teacherToggleFollowerUrl, followersTrans) {
  var posting = $.post(teacherToggleFollowerUrl, {
    pk: pk
  });
  posting.done(function (data) {
    var followerValue = parseInt(document.getElementById("follower-count-" + pk).innerHTML.substring(11));
    console.log(data);
...
function init$1(collectionDetailUrl, toggleFollowerUrl, followersStr) {
  $.ajaxSetup({
    beforeSend: function beforeSend(xhr, settings) {
      if (!bundle.csrfSafeMethod(settings.type) && !this.crossDomain) {
        xhr.setRequestHeader("X-CSRFToken", bundle.getCsrfToken());
  ...
  [].forEach.call(document.querySelectorAll(".follower-btn"), function (el) {
    el.addEventListener("click", function () {
      toggleFollower(el.getAttribute('pk'), toggleFollowerUrl, followersStr);
    });
  });
  [].forEach.call(document.querySelectorAll(".detail-view"), function (el) {
    el.addEventListener("click", function () {
...
function init$2(collectionToggleAssignmentUrl) {
  ajaxJQSetup();
  [].forEach.call(document.querySelectorAll(".mdc-icon-toggle"), function (el) {
    bundle.iconToggle.MDCIconToggle.attachTo(el);
  });
...
exports.initCollectionDistribute = init;
exports.initCollectionList = init$1;
exports.initCollectionUpdate = init$2;
