...
class MDCSnackbarFoundation extends MDCFoundation {
  ...
  constructor(adapter) {
    ...
    this.visibilitychangeHandler_ = () => {
      ...
      if (!this.adapter_.visibilityIsHidden()) {
        setTimeout(this.cleanup_.bind(this), this.snackbarData_.timeout || numbers$5.MESSAGE_TIMEOUT);
      }
    };
  ...
  show(data) {
    ...
    const {ACTIVE, MULTILINE, ACTION_ON_BOTTOM} = cssClasses$18;
...
function transformer$1() {
  ...
  scale.domain = function(_) {
    return arguments.length ? (domain = map$4.call(_, number$2), clamp === identity$7 || (clamp = clamper(domain)), rescale()) : domain.slice();
  };
...
function bindAjaxTextInputForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook) {
  ...
  if (d) {
    d.onclick = function () {
      ...
      function callback() {
        ...
        input.focus();
        init(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook);
      }
...
function categoryForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook) {
  // Define ENTER key
  var form = $("#category_form").find("#id_title");
  ...
  $("#clear_category_form").click(function () {
    $("#category_form").load(formUrl, function () {
      bundle.bindAjaxTextInputForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook);
  ...
  $("#submit_category_form").click(function () {
    var title = $("#category_form").find("input[name='title']").val(); // Send the data using post

    var posting = $.post(createUrl, {
      title: title
    }); // Put the results in a div

    posting.success(function (data, status) {
      $("#category_form").empty().append(data);
      var formType = $("#create_new_category");

      if (formType.length) {
        categoryForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook);
        bundle.autoInit();
...
function bindCategoryAutofill(source) {
  ...
  var response = function response(searchClass, spinnerId) {
    return function (event, ui) {
      // NB: Pass by reference.  ui can be modified, but not recreated.
      var currentList = $.map($(searchClass), function (obj, i) {
        return $(obj).attr("d");
      });
      var tmp = ui.content.filter(function (el) {
        return !currentList.includes(el.label);
      });
      var l = ui.content.length;
  ...
  var search = function search(spinnerId) {
    return function (event, ui) {
  ...
  var select = function select(currentIds, className, formId) {
    return function (event, ui) {
      event.preventDefault();
...
function disciplineForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook) {
  ...
  $("#submit_discipline_form").click(function () {
    var title = $("#discipline_form").find("input[name='title']").val(); // Send the data using post
    ...
    posting.success(function (data, status) {
      ...
      var formType = $("#discipline_create_form");

      if (formType.length) {
        disciplineForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook);
        bundle.autoInit();
      } else {
        bundle.bindAjaxTextInputForm(idToBind, formToReplace, createUrl, formUrl, init, searchUrl, completionHook);
...
function plot(matrix, freq, id) {
  ...
  g.append("text").attr("x", size / 4).attr("y", size / 4).attr("dy", 4).style("font-size", "8pt").style("fill", "white").style("text-anchor", "middle").attr("pointer-events", "none").text(parseInt(100 * matrix["easy"]) + "%");
  ...
  g.append("text").attr("x", 3 * size / 4).attr("y", 3 * size / 4).attr("dy", 4).style("font-size", "8pt").style("fill", "white").style("text-anchor", "middle").attr("pointer-events", "none").text(parseInt(100 * matrix["hard"]) + "%");
  ...
  g.append("text").attr("x", size / 4).attr("y", 3 * size / 4).attr("dy", 4).style("font-size", "8pt").style("fill", "white").style("text-anchor", "middle").attr("pointer-events", "none").text(parseInt(100 * matrix["peer"]) + "%");
  ...
  g.append("text").attr("x", 3 * size / 4).attr("y", size / 4).attr("dy", 4).style("font-size", "8pt").style("fill", "white").style("text-anchor", "middle").attr("pointer-events", "none").text(parseInt(100 * matrix["tricky"]) + "%");
...
exports.bindCategoryAutofill = bindCategoryAutofill;
exports.disciplineForm = disciplineForm;
