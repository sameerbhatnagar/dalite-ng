...
function handleDragEnd(event) {
  ...
  Array.from(elem.parentNode.getElementsByClassName("draggable")).map(x => x.classList.remove("draggable--over"));
}

function handleDragEnter(event) {
  const elem = event.currentTarget;
  const container = elem.parentNode;
  const title = event.dataTransfer.getData("title") || window.currentDraggedName;
  const oldElem = Array.from(container.children).filter(x => x.getAttribute("data-draggable-name") == title)[0];
...
function addEventListeners() {
  Array.from(document.getElementsByClassName("foldable--title")).map(x => x.addEventListener("click", e => toggleFoldable(e)));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("dragstart", e => handleDragStart(e), false));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("dragend", e => handleDragEnd(e), false));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("dragenter", e => handleDragEnter(e), false));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("dragleave", e => handleDragLeave(e), false));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("dragover", e => handleDragOver(e), false));
  Array.from(document.getElementsByClassName("draggable")).map(x => x.addEventListener("drop", e => handleDrop(e), false));
}
...
function getCookie(name) {
  ...
  if (document.cookie && document.cookie != "") {
    ...
    for (let i = 0; i < cookies.length; i++) {
      const cookie = jQuery.trim(cookies[i]); // Does this cookie string begin with the name we want?
...
function cornerGraphic(svgSelector, formID, lang, className) {
  ...
  g.append("path").attr("class", className).attr("d", () => {
    const path_ = path$1();
    path_.moveTo(0, h);
    path_.lineTo(w, 0);
    path_.lineTo(w, h);
...
function wrap(text, width) {
  ...
  function () {
    ...
    let tspan = text.text(null).append("tspan").attr("x", x).attr("y", y).attr("dx", dx).attr("dy", dy + "px");
...
function underlines() {
  ...
  const lines = selectAll(".underline");
  lines.selectAll("g").remove();
  const w = document.querySelector("main").offsetWidth;
  const gradientX = lines.append("linearGradient").attr("id", "underlineGradientX").attr("x1", 0).attr("x2", 1).attr("y1", 0).attr("y2", 0);
  gradientX.append("stop").attr("offset", "0%").attr("stop-color", "#54c0db");
  gradientX.append("stop").attr("offset", "100%").attr("stop-color", "#004266");
  const gradientY = lines.append("linearGradient").attr("id", "underlineGradientY").attr("x1", 0).attr("x2", 0).attr("y1", 0).attr("y2", 1);
...
function plot(matrix, freq, id) {
  ...
  const easy = g.append("rect").attr("x", 0).attr("y", 0).attr("width", size / 2).attr("height", size / 2).attr("fill", colour["easy"]).style("opacity", 0.5 + 0.5 * matrix["easy"]);
  easy.on("mousemove", () => {
  ...
  const hard = g.append("rect").attr("x", size / 2).attr("y", size / 2).attr("width", size / 2).attr("height", size / 2).attr("fill", colour["hard"]).style("opacity", 0.5 + 0.5 * matrix["hard"]);
  hard.on("mousemove", () => {
  ...
  const peer = g.append("rect").attr("x", 0).attr("y", size / 2).attr("width", size / 2).attr("height", size / 2).attr("fill", colour["peer"]).style("opacity", 0.5 + 0.5 * matrix["peer"]);
  peer.on("mousemove", () => {
  ...
  const tricky = g.append("rect").attr("x", size / 2).attr("y", 0).attr("width", size / 2).attr("height", size / 2).attr("fill", colour["tricky"]).style("opacity", 0.5 + 0.5 * matrix["tricky"]);
  tricky.on("mousemove", () => {
  ...
  const y = bundle.scaleBand().domain(bundle.keys(freq["first_choice"]).sort()).rangeRound([0, firstFreqSvg.attr("height")]);
  const gg = secondFreqSvg.append("g").attr("transform", "translate(" + margin.left + ",0)");
...
function plotTimeSeries(el, d) {
  ...
  const x = time().domain([new Date(timeParse(d.distribution_date)), new Date(timeParse(d.due_date))]).range([0, width]);
  const y = linear$2().domain([0, d.total]).range([height, 0]);
  const xAxis = axisBottom(x);
  const xAxisTop = axisTop(x).ticks("");
  const g = svg.append("g");
  ...
  if (d.due_date > d.last_login) {
    const endDate = Math.min(new Date(timeParse(d.now)), new Date(timeParse(d.due_date)));
  ...
  const area = area$3().x0(f.x()).y0(height).x1(f.x()).y1(f.y()).curve(stepAfter);
  svg.on("mousemove",
  /* @this */
  function () {
    const xValue = Math.min(mouse(this)[0], 1 + x(max(d.answers.map(x => new Date(timeParse(x))))));
    ...
    g.select(".slider-label-top").attr("x", xValue).text(parseInt(100 * bisectLeft(d.answers.map(x => new Date(timeParse(x))), x.invert(xValue)) / d.total) + "%");
    let data = d.answers.map(x => new Date(timeParse(x)));
    const index = bisectLeft(data, x.invert(xValue));
