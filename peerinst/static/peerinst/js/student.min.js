...
function copyStudentIdToClipboard(group, node) {
  navigator.clipboard.writeText(group.studentId).then(() => showCopyBubble(node));
}
...
function joinGroup() {
  ...
  const req = buildReq(data, "post");
  fetch(url, req).then(resp => resp.json()).then(function (group) {
    input.value = "";

    if (model.groups.some(g => g.name === group.name)) {
      model.groups.filter(g => g.name === group.name)[0].memberOf = group.member_of;
    } else {
      model.groups.push({
        name: group.name,
        title: group.title,
...
function identityView() {
  ...
  const tosSharingSpan = document.getElementById("student-tos-sharing--sharing");
...
function joinGroupsSelectView() {
  ...
  if (oldGroups.length) {
    oldGroups.map(group => groupsSelect.appendChild(joinGroupSelectView(group)));
...
function groupsView(groupStudentId) {
  ...
  clear(groups);
  model.groups.filter(group => group.memberOf).map(group => groups.appendChild(groupView(group)));
...
function groupTitleIdView(group) {
  ...
  copyIcon.addEventListener("click", () => copyStudentIdToClipboard(group, div));
  ...
  input.addEventListener("keydown", event => handleStudentIdKeyDown(event.key, group, div));
...
function groupAssignmentsView(group) {
  ...
  if (group.assignments.length) {
    ...
    group.assignments.map(assignment => ul.appendChild(groupAssignmentView(assignment, group)));
    div.appendChild(ul);
  } else {
...
function groupAssignmentView(assignment, group) {
  ...
  li.addEventListener("click", event => goToAssignment(group, assignment));
  ...
  if (assignment.done) {
    ...
    remainingTimeSpan.textContent = timeuntil(assignment.dueDate, new Date(Date.now()), true);
...
function toggleStudentIdNeededView(group) {
  ...
  const node = document.querySelector(`.student-group[data-group='${group.name}'] .student-group--id`);
  ...
  if (((_node$querySelector = node.querySelector(".student-group--id__input")) === null || _node$querySelector === void 0 ? void 0 : _node$querySelector.style.display) === "none") {
    ...
    alert === null || alert === void 0 ? void 0 : alert.parentNode.removeChild(alert);
  }
}
...
function initStudentPage(data, groupStudentId = "") {
  initModel(data);
  view(groupStudentId);
}
...
function markNotificationRead(notification) {
  const url = model$1.urls.removeNotification;
  const data = {
    notification_pk: notification.pk
  };
  const req = buildReq(data, "post");
  fetch(url, req).then(resp => resp.text()).then(groupName => {
    if (groupName) {
...
function markAllNotificationsRead() {
  const url = model$1.urls.removeNotifications;
...
function initStudentHeader(url) {
...
exports.initStudentHeader = initStudentHeader;
