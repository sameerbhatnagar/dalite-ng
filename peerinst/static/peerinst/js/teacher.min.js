...
function toggleNotifications() {
  model$2.notificationsOpen = !model$2.notificationsOpen;
  notificationsView();
}

function updateNotifications(notifications) {
  model$2.notifications = notifications;
  notificationsView();
}
/********/
...
function notificationsView() {
  ...
  var completedNotifications = model$2.notifications.filter(function (notification) {
  ...
  if (model$2.notifications.length) {
    model$2.notifications.map(function (notification) {
  ...
  if (model$2.notifications.some(function (notification) {
    return notification.inProgress;
  })) {
...
function _initTasks() {
  _initTasks = _asyncToGenerator(function* (data) {
    model$1.tasks = data.map(function (task) {
      return {
        id: task.id,
    ...
    Promise.all(model$1.tasks.filter(function (task) {
...
function _requestGradebook() {
  _requestGradebook = _asyncToGenerator(function* (event) {
    ...
    var url = model$1.urls.requestGradebook;
    var req = buildReq(data, "post");
...
function _getGradebookResult() {
  _getGradebookResult = _asyncToGenerator(function* (task) {
    var url = model$1.urls.gradebookResult;
    var req = buildReq({
      task_id: task.id
    }, "post");
    var resp = yield fetch(url, req);
...
function _removeGradebookError() {
  _removeGradebookError = _asyncToGenerator(function* (task) {
    var url = model$1.urls.removeFailedGradebook;
    var req = buildReq({
      task_id: task.id
    }, "post");
    var resp = yield fetch(url, req);
    ...
    if (resp.ok) {
      model$1.tasks = model$1.tasks.filter(function (t) {
        return t.id !== task.id;
      });
    }
...
function _downloadGradebook2() {
  _downloadGradebook2 = _asyncToGenerator(function* (task) {
    ...
    var url = model$1.urls.downloadGradebook;
    var req = buildReq(data, "post");
    var resp = yield fetch(url, req);
...
function getNotifications() {
  return model$1.tasks.map(function (task) {
    return {
...
function toggleMessages() {
  model$3.open = !model$3.open;
  messagesView$1();
}

function getMessages$1() {
  var url = model$3.urls.messagesUrl;
  var req = buildReq({}, "get");
  fetch(url, req).then(function (resp) {
    return resp.json();
  }).then(function (data) {
    model$3.messages = data.threads.map(function (message) {
      return {
        id: message.id,
        title: message.title,
        lastReply: {
...
function _markAllRead() {
  _markAllRead = _asyncToGenerator(function* () {
    var req = buildReq({}, "post");
    var resp = yield fetch(model$3.urls.markReadUrl, req);

    if (resp.ok) {
      model$3.messages.forEach(function (message) {
        message.nNew = 0;
      });
    }
...
function messagesView$1() {
  ...
  var nNew = model$3.messages.filter(function (message) {
  ...
  if (model$3.messages.length) {
    model$3.messages.map(function (message) {
...
function initModel$4(flagQuestionUrl, getFlagQuestionReasonsUrl, getNewQuestionUrl, toggleFavouriteUrl) {
  model$4 = {
    flagQuestionReasons: [],
    urls: {
...
function _getFlagQuestionReasons() {
  _getFlagQuestionReasons = _asyncToGenerator(function* () {
    var req = buildReq({}, "get");
    var resp = yield fetch(model$4.urls.getFlagQuestionReasons, req);
    ...
    var data = yield resp.json();
    model$4.flagQuestionReasons = data.reasons;
    flagQuestionView();
  });
  return _getFlagQuestionReasons.apply(this, arguments);
}
...
function _flagQuestion() {
  _flagQuestion = _asyncToGenerator(function* (question) {
    ...
    var req = buildReq(data, "post");
    var resp = yield fetch(model$4.urls.flagQuestion, req);
...
function _getNewQuestion() {
  _getNewQuestion = _asyncToGenerator(function* (question) {
    var posting = $.get(model$4.urls.getNewQuestion);
    posting.done(function (data) {
      $(question).toggle("fade", function () {
        $(question).remove();
        $("#questions").append(data);
        $("#questions .mdc-card").hide().toggle("slide", transition);
        initNewCard(model$4.urls.toggleFavourite, model$4.urls.getNewQuestion);
...
function flagQuestionView() {
  _toConsumableArray(document.getElementsByClassName("flag-question")).forEach(function (question) {
    ...
    clear(select);
    model$4.flagQuestionReasons.forEach(function (reason) {
...
function addFlagQuestionListeners() {
  _toConsumableArray(document.getElementsByClassName("flag-question")).forEach(function (question) {
    question.querySelector(".flag-question__form__cancel").addEventListener("click", function (event) {
      event.stopPropagation();
      toggleFlagQuestion(question);
    });
...
function init$5(_x3, _x4, _x5, _x6) {
...
function _init$2() {
  _init$2 = _asyncToGenerator(function* (favUrl, refreshUrl, getFlagQuestionReasonsUrl, flagQuestionUrl) {
    initModel$4(flagQuestionUrl, getFlagQuestionReasonsUrl, refreshUrl, favUrl);
    initNewCard(favUrl, refreshUrl);
    update$3();
  });
  return _init$2.apply(this, arguments);
...
function init$7(url) {
  ...
  $("#rationales .mdc-card").each(function (i, el) {
    ...
    var flagBtn = document.getElementById("submit-flag-btn");
    ...
    function processResponse(data) {
      $(el).toggle("fade", function () {
        ...
        init$7(url); // Refresh reputation score
...
exports.initQuestionCards = init$5;
exports.initStudentActivityCards = init$6;
exports.initRationalesToScoreCards = init$7;
