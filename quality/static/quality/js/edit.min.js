...
function updateCriterionOption(event, option, criterion) {
  ...
  if (type === "CommaSepField" || type === "ManyToManyField") {
    ...
    if (event.key === "Enter" || event.key === "," || event.key === " ") {
      ...
      if (criterion[name].allowed) {
        if (!criterion[name].allowed.includes(value)) {
          toggleCriterionOptionError(option.querySelector(".comma-sep-input--input"), `${value} isn't an accepted language. Options are ${criterion[name].allowed.slice(0, criterion[name].allowed.length - 1).join(", ")} and ${criterion[name].allowed[criterion[name].allowed.length - 1]}.`);
    ...
    if (type === "ProbabilityField") {
      if (value < 0) {
        ...
        option.value = model.criterions.filter(c => c.name === criterion.name)[0][name].value;
        return;
      } else if (model.criterions.filter(c => c.name === criterion.name)[0][name].value == value) {
        if (value === "00") {
          option.value = "0";
        }
  ...
  fetch(model.urls.updateCriterion, req).then(resp => resp.ok ? resp.json() : resp.text()).then(data => {
    if (typeof data === "string") {
      toggleCriterionOptionError(err);
    } else {
      model.criterions = model.criterions.map(c => c.name === data.name ? data : c);
...
function addCriterion(criterion) {
  ...
  const req = buildReq(data, "post");
  fetch(model.urls.addCriterion, req).then(resp => resp.json()).then(json => {
...
function removeCriterion(criterion) {
  ...
  const req = buildReq(data, "post");
  fetch(model.urls.removeCriterion, req).then(resp => {
    if (resp.ok) {
      model.criterions = model.criterions.filter(c => c.name != criterion.name);
      criterionsView();
      newCriterionsView();
    }
  }).catch(err => console.log(err));
...
function criterionView(criterion) {
  ...
  const binaryThreshold = criterion.versions[criterion.version - 1].binary_threshold; // const versionLabel = document.createElement("label");
  ...
  const weight = criterionOptionView("PositiveIntegerField", criterion.weight, criterion);
  weight.name = "weight";
  options.appendChild(weightLabel);
  options.appendChild(weight);
  const otherOptions = Object.keys(criterion).filter(o => !["description", "full_name", "is_beta", "name", "version", "versions", "weight"].includes(o));
  otherOptions.forEach(o => {
    if (!binaryThreshold || o !== "threshold") {
      ...
      const input = criterionOptionView(option.type, option.value, criterion, null);
...
function criterionOptionView(type, value, criterion, input = null) {
  ...
  if (type === "PositiveIntegerField") {
    if (!input) {
      ...
      input.type = "number";
      input.min = 0;
      input.addEventListener("input", event => updateCriterionOption(event, input, criterion));
    }
    ...
    input_.type = "text";
    input_.addEventListener("keydown", event => updateCriterionOption(event, input, criterion));
    ...
    if (!input) {
      ...
      input_.addEventListener("click", event => updateCriterionOption(event, input, criterion));
      input.appendChild(input_);
...
function newCriterionsView() {
  ...
  const available = model.available.filter(c => !model.criterions.map(cc => cc.name).includes(c.name));
