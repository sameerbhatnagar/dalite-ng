    ...
    if (event.key === "Enter" || event.key === "," || event.key === " ") {
      ...
      if (criterion[name].allowed) {
        if (!criterion[name].allowed.includes(value)) {
          toggleCriterionOptionError(option.querySelector(".comma-sep-input--input"), "".concat(value, " isn't an accepted language. Options are ").concat(criterion[name].allowed.slice(0, criterion[name].allowed.length - 2).join(", "), " and ").concat(criterion[name].allowed[criterion[name].allowed.length - 1], "."));
